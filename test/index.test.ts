...
import { AwaitTimeout, Erisa } from '..';
  ...
  describe('Erisa', () => {
    describe('#use', () => {
      ...
      describe('registering under one event', () => {
        for (const [name, tester] of Object.entries(tests))
          specify(name, () => {
            if (['rest handlers', 'mix of handlers and arrays'].includes(name))
              client.use('foo', ...(tester as Array<VoidFunc | VoidFunc[]>));
            else client.use('foo', tester as VoidFunc | VoidFunc[]);

            // Everything other than the first test should be equal to `handlers`.
            expect(client.handlers.get('foo')).to.deep.equal(
              name === 'single handler' ? [tester] : handlers
            );
          });
      });
      ...
      describe('registering under an event array', () => {
        for (const [name, tester] of Object.entries(tests))
          specify(name, () => {
            if (['rest handlers', 'mix of handlers and arrays'].includes(name))
              client.use(events, ...(tester as Array<VoidFunc | VoidFunc[]>));
            else client.use(events, tester as VoidFunc | VoidFunc[]);

            for (const ev of events)
              expect(client.handlers.get(ev)).to.deep.equal(
                name === 'single handler' ? [tester] : handlers
              );
          });
      });
      ...
      describe('implicitly registering under the `*` event', () => {
        for (const [name, tester] of Object.entries(tests))
          specify(name, () => {
            if (['rest handlers', 'mix of handlers and arrays'].includes(name))
              client.use(...(tester as Array<VoidFunc | VoidFunc[]>));
            else client.use(tester as VoidFunc | VoidFunc[]);

            expect(client.handlers.get('*')).to.deep.equal(
              name === 'single handler' ? [tester] : handlers
            );
          });
      });
      ...
      it('should register pairs of events and middleware correctly', () => {
        ...
        expect(client.handlers.get('bar')).to.deep.equal([
          handlers[0],
          handlers[1]
        ]);
      ...
      describe('removing from explicitly defined events', () => {
        for (const [name, tester] of Object.entries(tests))
          specify(name, () => {
            const result = [
              'rest handlers',
              'mix of handlers and arrays'
            ].includes(name)
              ? (tester as VoidFunc[])
              : ([tester] as [VoidFunc | VoidFunc[]]);
      ...
      describe("removing all handlers from existance on any event they're applied on", () => {
        for (const [name, tester] of Object.entries(tests))
          specify(name, () => {
            ...
            for (const ev of events)
              expect(client.handlers.get(ev)).to.be.undefined;
          });
      });
    });
    ...
    describe('#format', () => {
      it('should throw an error when given an unknown object', () => {
        const FakeClass = class {}; // eslint-disable-line @typescript-eslint/no-extraneous-class
        const format = client.format.bind(client);

        expect(curry(format, null as any)).to.throw(
          TypeError,
          'Unable to format object: null'
        );
        expect(curry(format, undefined as any)).to.throw(
          TypeError,
          'Unable to format object: undefined'
        );
        expect(curry(format, false as any)).to.throw(
          TypeError,
          'Unable to format object: false'
        );
        expect(curry(format, new FakeClass() as any)).to.throw(
          TypeError,
          `Unable to format object: ${FakeClass.name}`
        );
      });
      ...
      describe('formatting objects', () => {
        ...
        describe('roles', () => {
          const role = new Eris.Role(
            { id: '1234567890', name: 'foo', mentionable: true },
            {} as any
          );
    ...
    describe('#awaitMessage', () => {
      ...
      it('should obey the given timeout, and timeout', () => {
        ...
        return expect(ret).to.eventually.be.rejectedWith(
          AwaitTimeout,
          'Message await expired.'
        );
      });
    });
  });
